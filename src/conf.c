...
unsigned char *dologname(unsigned char *buf, unsigned char *name,
                         const unsigned char *ext, ROTATION lt, time_t t) {
    struct tm *ts;
    ...
    if (strchr((char *)name, '%')) {
        ...
        case ANNUALLY:
            ...
            break;
        case MONTHLY:
            sprintf((char *)buf, "%s.%04d.%02d", name, ts->tm_year + 1900,
                    ts->tm_mon + 1);
            break;
        case WEEKLY:
            t = t - (ts->tm_wday * (60 * 60 * 24));
            ts = localtime(&t);
            sprintf((char *)buf, "%s.%04d.%02d.%02d", name, ts->tm_year + 1900,
        ...
        case DAILY:
            ...
            break;
        case HOURLY:
            sprintf((char *)buf, "%s.%04d.%02d.%02d-%02d", name,
                    ts->tm_year + 1900, ts->tm_mon + 1, ts->tm_mday,
                    ts->tm_hour);
            break;
        case MINUTELY:
            sprintf((char *)buf, "%s.%04d.%02d.%02d-%02d.%02d", name,
                    ts->tm_year + 1900, ts->tm_mon + 1, ts->tm_mday,
...
int start_proxy_thread(struct child *chp) {
...
#ifdef _WIN32
#ifndef _WINCE
    h = (HANDLE)_beginthreadex((LPSECURITY_ATTRIBUTES)NULL,
                               16384 + conf.stacksize, (void *)startsrv,
                               (void *)chp, (DWORD)0, &thread);
#else
    h = (HANDLE)CreateThread((LPSECURITY_ATTRIBUTES)NULL,
                             16384 + conf.stacksize, (void *)startsrv,
                             (void *)chp, (DWORD)0, &thread);
#endif
    if (h)
    ...
    pthread_attr_setstacksize(&pa,
                              PTHREAD_STACK_MIN + (32768 + conf.stacksize));
...
#else
    while (read(conf.threadinit[0], r, 1) != 1)
        if (errno != EINTR) {
    ...
    if (haveerror) {
        fprintf(stderr, "Service not started on line: %d%s\n", linenum,
                haveerror == 2 ? ": insufficient memory" : "");
        return (40);
    }
    return 0;
}

char **list_dir_content(char *path) {
...
static int h_proxy(int argc, unsigned char **argv) {
    ...
    if (!strcmp((char *)argv[0], "proxy")) {
        ...
        if (!resolvfunc || (resolvfunc == myresolver && !dns_table.hashsize)) {
            fprintf(stderr,
                    "[line %d] Warning: no nserver/nscache "
                    "configured, proxy may run "
                    "very slow\n",
                    linenum);
        }
#endif
        ...
        childdef.helpmessage =
            " -hdefault_host[:port] - use this host and port as "
            "default if no host specified\n";
        ...
        childdef.helpmessage = " -s single packet UDP service for "
                               "request/reply (DNS-like) services\n";
        ...
        childdef.helpmessage =
            " -s - simple DNS forwarding - do not use 3proxy "
            "resolver / name cache\n";
#ifndef NOIPV6
        if (!resolvfunc || (resolvfunc == myresolver && !dns_table.hashsize) ||
...
static int h_log(int argc, unsigned char **argv) {
    ...
    havelog = 1;
    if (argc > 1 && conf.logtarget &&
        !strcmp((char *)conf.logtarget, (char *)argv[1])) {
        notchanged = 1;
    }
    ...
    if (argc > 1) {
        ...
        if (*argv[1] == '@') {
            ...
            if (conf.stdlog)
                conf.stdlog =
                    freopen((char *)dologname(tmpbuf, conf.logname, NULL,
                                              conf.logtype, conf.logtime),
                            "a", conf.stdlog);
            else
                conf.stdlog =
                    fopen((char *)dologname(tmpbuf, conf.logname, NULL,
                                            conf.logtype, conf.logtime),
                          "a");
            if (!conf.stdlog) {
                perror((char *)tmpbuf);
...
static int h_service(int argc, unsigned char **argv) { return 0; }
...
static int h_includedir(int argc, unsigned char **argv) {
    char **entries = list_dir_content((char *)argv[1]);
    ...
    for (int i = 0; entries[i] != NULL; i++) {
        combine(dest, (char *)argv[1], entries[i]);

        unsigned char *fake_argv[2];
        fake_argv[0] = (unsigned char *)"include";
        fake_argv[1] = (unsigned char *)dest;

        h_include(2, fake_argv);
    }

    return 0;
}

int h_print(int argc, unsigned char **argv) {
    if (argc < 2)
        return 0;
...
static int h_archiver(int argc, unsigned char **argv) {
    ...
    if (conf.archiver) {
        conf.archiverc = argc;
        for (j = 0; j < conf.archiverc; j++)
            conf.archiver[j] = (unsigned char *)mystrdup((char *)argv[j]);
    }
    return 0;
}
...
#endif
#else
#define MAX_COUNTER_TIME                                                       \
    ((sizeof(time_t) > 4) ? (time_t)0x793406fff : (time_t)0x7fffffff)
#endif

        if (ch1.updated < 0 || ch1.updated >= MAX_COUNTER_TIME) {
            fprintf(stderr,
                    "Invalid or corrupted counter file %s. Use "
                    "countersutil utility "
                    "to convert from older version\n",
                    argv[1]);
            return 3;
        }
...
static int h_timeouts(int argc, unsigned char **argv) {
    ...
    for (j = 0; conf.timeouts[j] && j + 1 < argc; j++) {
        if ((conf.timeouts[j] = atoi((char *)argv[j + 1])) <= 0 ||
            conf.timeouts[j] > 2000000) {
...
static int h_noop(int argc, unsigned char **argv) { return 0; }
...
static int h_auth(int argc, unsigned char **argv) {
    ...
    conf.authfuncs = NULL;
    if (!conf.bandlimfunc)
        conf.bandlimfunc = bandlimitfunc;
    for (argc--; argc; argc--) {
...
#ifndef _WIN32
    {
        ...
        if (!getrlimit(RLIMIT_NOFILE, &rl)) {
            if ((conf.maxchild << 1) > rl.rlim_cur)
                fprintf(stderr,
                        "[line %d] Warning: current open file ulimits "
                        "are too low "
                        "(cur: %d/max: %d),"
                        " maxconn requires at least %d for every running "
                        "service."
                        " Configure ulimits according to system "
                        "documentation\n",
                        linenum, (int)rl.rlim_cur, (int)rl.rlim_max,
                        (conf.maxchild << 1));
        }
    }
#endif
...
static int h_nserver(int argc, unsigned char **argv) {
    ...
    if (numservers < MAXNSERVERS) {
        ...
        *SAPORT(&nservers[numservers].addr) = htons(53);
        if (parsehost(46, argv[1],
                      (struct sockaddr *)&nservers[numservers].addr))
            return 1;
        if (str) {
...
static int h_authnserver(int argc, unsigned char **argv) {
    ...
    if ((str = strchr((char *)argv[1], '/')))
        *str = 0;
    if (parsehost(46, argv[1], (struct sockaddr *)&authnserver.addr))
        return 1;
...
static int h_nsrecord(int argc, unsigned char **argv) {
    ...
    if (!getip46(46, argv[2], (struct sockaddr *)&sa))
        return 1;

    hashadd(*SAFAMILY(&sa) == AF_INET6 ? &dns6_table : &dns_table, argv[1],
            SAADDR(&sa), (time_t)0xffffffff);
    return 0;
}

static int h_dialer(int argc, unsigned char **argv) {
    if (conf.demanddialprog)
        myfree(conf.demanddialprog);
...
static int h_monitordir(int argc, unsigned char **argv) {
    char **entries = list_dir_content((char *)argv[1]);
    ...
    for (int i = 0; entries[i] != NULL; i++) {
        ...
        unsigned char *fake_argv[2];
        fake_argv[0] = (unsigned char *)"monitor";
        fake_argv[1] = (unsigned char *)dest;

        h_monitor(2, fake_argv);
    }

    return 0;
}

struct redirdesc redirs[] = {{R_TCP, "tcp", tcppmchild},
...
static int h_parent(int argc, unsigned char **argv) {
    ...
    if (!acl || (acl->action && acl->action != 2)) {
        fprintf(stderr,
                "Chaining error: last ACL entry was not \"allow\" or "
                "\"redirect\" "
                "on line %d\n",
                linenum);
        return (1);
    ...
    *SAPORT(&chains->addr) = htons((unsigned short)atoi((char *)argv[4]));
    if (argc > 5)
        chains->extuser = (unsigned char *)mystrdup((char *)argv[5]);
    if (argc > 6)
        chains->extpass = (unsigned char *)mystrdup((char *)argv[6]);
    if (!acl->chains) {
        acl->chains = chains;
    } else {
...
static int h_nolog(int argc, unsigned char **argv) {
    ...
    if (!acl) {
        fprintf(stderr,
                "Chaining error: last ACL entry was not \"allow/deny\" "
                "on line %d\n",
                linenum);
        return (1);
    }
...
int scanipl(unsigned char *arg, struct iplist *dst) {
    ...
    if ((slash = strchr((char *)arg, '/')))
        *slash = 0;
    if ((dash = strchr((char *)arg, '-')))
        *dash = 0;
    ...
    if (dash) {
        if (afdetect((unsigned char *)dash + 1) == -1)
            return 1;
        if (!getip46(46, (unsigned char *)dash + 1, (struct sockaddr *)&sa))
            return 2;
        memcpy(&dst->ip_to, SAADDR(&sa), SAADDRLEN(&sa));
        if (*SAFAMILY(&sa) != dst->family ||
            memcmp(&dst->ip_to, &dst->ip_from, SAADDRLEN(&sa)) < 0)
            return 3;
        return 0;
    }
    ...
    if (slash) {
        *slash = '/';
        ...
        if (masklen < 0 || masklen > (addrlen * 8))
            ...
            for (; nbits; nbits--) {
                ((unsigned char *)&dst->ip_from)[nbytes] &=
                    ~(0x01 << (nbits - 1));
...
struct ace *make_ace(int argc, unsigned char **argv) {
        ...
        do {
            ...
            if (scanipl(arg, &tmpip)) {
                if (!arglen)
                    continue;
                if (!acl->dstnames) {
                    acl->dstnames = hostnamel =
                        myalloc(sizeof(struct hostname));
                } else {
    ...
    if (argc > 3 && strcmp("*", (char *)argv[3])) {
        ...
        do {
            ...
            res = sscanf((char *)arg, "%hu-%hu", &portl->startport,
                         &portl->endport);
            if (res < 1) {
    ...
    if (argc > 6) {
        for (arg = argv[6];
             strlen((char *)arg) >= 17 && isdigit(arg[0]) && isdigit(arg[1]) &&
             isdigit(arg[3]) && isdigit(arg[4]) && isdigit(arg[6]) &&
             isdigit(arg[7]) && isdigit(arg[9]) && isdigit(arg[10]) &&
             isdigit(arg[12]) && isdigit(arg[13]) && isdigit(arg[15]) &&
             isdigit(arg[16]);
             arg += 18) {
...
static int h_ace(int argc, unsigned char **argv) {
    ...
    if (!strcmp((char *)argv[0], "allow")) {
        ...
        res = REDIRECT;
        offset = 2;
    } else if (!strcmp((char *)argv[0], "bandlimin") ||
               !strcmp((char *)argv[0], "bandlimout")) {
        res = BANDLIM;
        offset = 1;
    } else if (!strcmp((char *)argv[0], "nobandlimin") ||
               !strcmp((char *)argv[0], "nobandlimout")) {
        res = NOBANDLIM;
    ...
    case REDIRECT:
        ...
        if (!getip46(46, argv[1], (struct sockaddr *)&acl->chains->addr))
            return 5;
        *SAPORT(&acl->chains->addr) =
            htons((unsigned short)atoi((char *)argv[2]));
    ...
    case NOBANDLIM:
        ...
        if (!strcmp((char *)argv[0], "bandlimin") ||
            !strcmp((char *)argv[0], "nobandlimin")) {
            if (!conf.bandlimiter) {
    ...
    case COUNTALL:
    case NOCOUNTALL:
        if (!conf.trafcountfunc)
            conf.trafcountfunc = trafcountfunc;
            ...
            if (tl->number != 0 && conf.counterd >= 0) {
                lseek(conf.counterd,
                      sizeof(struct counter_header) +
                          (tl->number - 1) * sizeof(struct counter_record),
                      SEEK_SET);
                ...
                if (tl->cleared < 0 || tl->cleared >= MAX_COUNTER_TIME ||
                    tl->updated < 0 || tl->updated >= MAX_COUNTER_TIME) {
                    fprintf(stderr, "Invalid, incompatible or "
...
static int h_logdump(int argc, unsigned char **argv) {
    ...
    if (argc > 2)
        conf.logdumpcli = (unsigned)atoi((char *)*(argv + 2));
    return 0;
}
...
static int h_radius(int argc, unsigned char **argv) {
    ...
    /*
            ...
            nradservers = 0;
            for(; oldrad; oldrad--){
                    if(radiuslist[oldrad].logsock >= 0)
    so._closesocket(radiuslist[oldrad].logsock); radiuslist[oldrad].logsock =
    -1;
            }
    */
    memset(radiuslist, 0, sizeof(radiuslist));
    if (strlen((char *)argv[1]) > 63)
        argv[1][63] = 0;
    strcpy(radiussecret, (char *)argv[1]);
    for (nradservers = 0; nradservers < MAXRADIUS && nradservers < argc - 2;
         nradservers++) {
        char *s = 0;
        ...
        if (!getip46(46, argv[nradservers + 2],
                     (struct sockaddr *)&radiuslist[nradservers].authaddr))
            return 1;
        if (s &&
            !getip46(46, (unsigned char *)s + 1,
                     (struct sockaddr *)&radiuslist[nradservers].localaddr))
            return 2;
        if (!*SAPORT(&radiuslist[nradservers].authaddr))
            *SAPORT(&radiuslist[nradservers].authaddr) = htons(1812);
        ...
        *SAPORT(&radiuslist[nradservers].logaddr) = htons(port + 1);
        /*
                        ...
                        if ((radiuslist[nradservers].logsock =
           so._socket(SASOCK(&radiuslist[nradservers].logaddr), SOCK_DGRAM,
           0)) < 0) return 2; if (so._bind(radiuslist[nradservers].logsock,
           (struct sockaddr
           *)&bindaddr, SASIZE(&bindaddr))) return 3;
        */
    }
    return 0;
}
...
static int h_authcache(int argc, unsigned char **argv) {
    ...
    if (strstr((char *)*(argv + 1), "ip"))
        conf.authcachetype |= 1;
    if (strstr((char *)*(argv + 1), "user"))
        conf.authcachetype |= 2;
    if (strstr((char *)*(argv + 1), "pass"))
        conf.authcachetype |= 4;
    if (strstr((char *)*(argv + 1), "limit"))
        conf.authcachetype |= 8;
    if (strstr((char *)*(argv + 1), "acl"))
        conf.authcachetype |= 16;
    if (strstr((char *)*(argv + 1), "ext"))
        conf.authcachetype |= 32;
    if (argc > 2)
        conf.authcachetime = (unsigned)atoi((char *)*(argv + 2));
    if (!conf.authcachetype)
        conf.authcachetype = 6;
    if (!conf.authcachetime)
        conf.authcachetime = 600;
    return 0;
}
...
struct commands commandhandlers[] = {
    ...
    {commandhandlers + 67, "includedir", h_includedir, 2, 2},
    {commandhandlers + 68, "monitordir", h_monitordir, 2, 2},
    {commandhandlers + 69, "print", h_print, 0, sizeof(char*)},

#ifndef NORADIUS
    {commandhandlers + 70, "radius", h_radius, 3, 0},
#endif
            ...
            if (!comment) {
                *str = 0;
                ...
                if (incbegin) {
                    ...
                    (*inbuf) += (res + len + 1);
                    included++;
                    argc += parsestr(incbegin, argm + argc, nitems - argc, buff,
                                     inbuf, bufsize);
                    included--;
                    incbegin = NULL;
                }
...
static void add_macro(const char *key, const char *value) {
    if (macro_count < MAX_MACROS) {
        strncpy(macros[macro_count].key, key, MAX_KEYLEN - 1);
        macros[macro_count].key[MAX_KEYLEN - 1] = '\0';
        strncpy(macros[macro_count].value, value, MAX_VALUELEN - 1);
        macros[macro_count].value[MAX_VALUELEN - 1] = '\0';
        macro_count++;
    }
}

static void parse_define_line(char *line) {
    char *p = line + 7;
    while (*p == ' ' || *p == '\t')
        p++;
...
static void macro_replace(char **line_ptr) {
    char *line = *line_ptr;
    ...
    do {
        ...
        for (int i = 0; i < macro_count; i++) {
            char *pos = strstr(result, macros[i].key);
            if (pos) {
                size_t pre_len = pos - result;
                size_t key_len = strlen(macros[i].key);
                size_t val_len = strlen(macros[i].value);
                size_t new_len = strlen(result) - key_len + val_len;

                if (new_len + 1 > bufsize) {
                    bufsize = new_len + 1;
                    ...
                    result = tmp;
                    pos = strstr(result, macros[i].key); // пересчёт позиции
                }

                memmove(result + pre_len + val_len, pos + key_len,
                        strlen(pos + key_len) + 1);
                memcpy(result + pre_len, macros[i].value, val_len);
        ...
        safety_counter++;
             ...
             safety_counter < 100); // ограничение, чтобы не зациклиться
...
int readconfig(FILE *fp) {
    ...
    if (!(buf = myalloc(bufsize)) ||
        !(argv = myalloc((NPARAMS + 1) * sizeof(unsigned char *)))) {
    ...
    for (linenum = 1; fgets((char *)buf, STRINGBUF, fp); linenum++) {
        ...
        if (strncmp((char *)buf, "define ", 7) == 0) {
            parse_define_line((char *)buf);
            continue;
        }

        // 1) применяем макросы на копии строки
        char *line = strdup((char *)buf);
        if (!line) {
        ...
        // 2) гарантируем, что рабочий buf вмещает line, и копируем line -> buf
        size_t need = strlen(line) + 1;
        if (need > (size_t)bufsize) {
            unsigned char *nbuf = myrealloc(buf, need);
            if (!nbuf) {
                fprintf(stderr, "No memory to expand buffer\n");
                free(line);
                myfree(buf);
                myfree(argv);
                return 10;
        ...
        // 3) парсим РОВНО как в исходнике — на buf, с &buf (пусть parsestr сам
        // realloc-ит buf)
        ...
        for (cm = commandhandlers; cm; cm = cm->next) {
            if (!strcmp((char *)argv[0], (char *)cm->command) &&
                argc >= cm->minargs && (!cm->maxargs || argc <= cm->maxargs)) {
                res = (*cm->handler)(argc, argv);
                if (res > 0) {
                    fprintf(stderr,
                            "Command: '%s' failed with code %d, line %d\n",
                            argv[0], res, linenum);
                    free(line);
                    return (linenum);
                }
...
void freeconf(struct extparam *confp) {
    ...
    *SAFAMILY(&confp->extsa) = AF_INET;
    ...
    for (; blout; blout = (struct bandlim *)itfree(blout, blout->next))
        freeacl(blout->ace);
    for (; cl; cl = (struct connlim *)itfree(cl, cl->next))
        freeacl(cl->ace);
