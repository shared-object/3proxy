...
unsigned char *dologname(unsigned char *buf, unsigned char *name,
  ...
  if (strchr((char *)name, '%')) {
    ...
    case DAILY:
      ...
      break;
    case HOURLY:
      sprintf((char *)buf, "%s.%04d.%02d.%02d-%02d", name, ts->tm_year + 1900,
...
#else
  h = (HANDLE)CreateThread((LPSECURITY_ATTRIBUTES)NULL, 16384 + conf.stacksize,
                           (void *)startsrv, (void *)chp, (DWORD)0, &thread);
#endif
  if (h)
    CloseHandle(h);
#else
...
static int h_proxy(int argc, unsigned char **argv) {
  ...
  if (!strcmp((char *)argv[0], "proxy")) {
    ...
    if (!resolvfunc || (resolvfunc == myresolver && !dns_table.hashsize)) {
      fprintf(stderr,
              "[line %d] Warning: no nserver/nscache configured, proxy may run "
              "very slow\n",
              linenum);
    }
#endif
    ...
    childdef.helpmessage = " -hdefault_host[:port] - use this host and port as "
    ...
    if (!resolvfunc || (resolvfunc == myresolver && !dns_table.hashsize)) {
      fprintf(stderr,
              "[line %d] Warning: no nserver/nscache configured, socks may run "
              "very slow\n",
              linenum);
    }
#endif
    ...
    childdef.helpmessage = " -s - simple DNS forwarding - do not use 3proxy "
                           "resolver / name cache\n";
#ifndef NOIPV6
    if (!resolvfunc || (resolvfunc == myresolver && !dns_table.hashsize) ||
        resolvfunc == fakeresolver) {
      fprintf(stderr,
              "[line %d] Warning: no nserver/nscache configured, dnspr will "
...
static int h_log(int argc, unsigned char **argv) {
  ...
  if (argc > 1) {
    ...
    if (*argv[1] == '@') {
      ...
      if (conf.stdlog)
        conf.stdlog = freopen((char *)dologname(tmpbuf, conf.logname, NULL,
                                                conf.logtype, conf.logtime),
                              "a", conf.stdlog);
      else
        conf.stdlog = fopen((char *)dologname(tmpbuf, conf.logname, NULL,
                                              conf.logtype, conf.logtime),
                            "a");
      if (!conf.stdlog) {
        perror((char *)tmpbuf);
...
#define MAX_COUNTER_TIME                                                       \
  ((sizeof(time_t) > 4) ? (time_t)0x793406fff : (time_t)0x7fffffff)
    ...
    if (ch1.updated < 0 || ch1.updated >= MAX_COUNTER_TIME) {
      fprintf(stderr,
              "Invalid or corrupted counter file %s. Use countersutil utility "
...
#ifndef _WIN32
  {
    ...
    if (!getrlimit(RLIMIT_NOFILE, &rl)) {
      if ((conf.maxchild << 1) > rl.rlim_cur)
        fprintf(stderr,
                "[line %d] Warning: current open file ulimits are too low "
                "(cur: %d/max: %d),"
...
static int h_nserver(int argc, unsigned char **argv) {
  ...
  if (numservers < MAXNSERVERS) {
    ...
    *SAPORT(&nservers[numservers].addr) = htons(53);
    if (parsehost(46, argv[1], (struct sockaddr *)&nservers[numservers].addr))
      return 1;
    if (str) {
...
static int h_parent(int argc, unsigned char **argv) {
  ...
  if (!acl || (acl->action && acl->action != 2)) {
    fprintf(stderr,
            "Chaining error: last ACL entry was not \"allow\" or \"redirect\" "
            "on line %d\n",
            linenum);
    return (1);
  }
...
static int h_nolog(int argc, unsigned char **argv) {
  ...
  if (!acl) {
    fprintf(
        stderr,
        "Chaining error: last ACL entry was not \"allow/deny\" on line %d\n",
        linenum);
    return (1);
  }
  while (acl->next)
...
static int h_radius(int argc, unsigned char **argv) {
  ...
  so._closesocket(radiuslist[oldrad].logsock); radiuslist[oldrad].logsock = -1;
          }
  */
  ...
  for (nradservers = 0; nradservers < MAXRADIUS && nradservers < argc - 2;
    ...
    if (!getip46(46, argv[nradservers + 2],
      ...
      return 1;
    if (s && !getip46(46, (unsigned char *)s + 1,
    ...
    *SAPORT(&radiuslist[nradservers].logaddr) = htons(port + 1);
    /*
       ...
       so._socket(SASOCK(&radiuslist[nradservers].logaddr), SOCK_DGRAM, 0)) < 0)
       return 2; if (so._bind(radiuslist[nradservers].logsock, (struct sockaddr
...
#ifndef NORADIUS
    {commandhandlers + 69, "radius", h_radius, 3, 0},
#endif
      ...
      if (!comment) {
        *str = 0;
        ...
        if (incbegin) {
          ...
          (*inbuf) += (res + len + 1);
          included++;
          argc += parsestr(incbegin, argm + argc, nitems - argc, buff, inbuf,
                           bufsize);
          included--;
          incbegin = NULL;
        }
